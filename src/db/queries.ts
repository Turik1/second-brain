...
export async function getThoughtStats(days = 30): Promise<{
  ...
  return {
    ...
    byType: Object.fromEntries(typeResult.rows.map((r: any) => [r.thought_type, parseInt(r.count)])),
    topTopics: topicsResult.rows.map((r: any) => ({ topic: r.topic, count: parseInt(r.count) })),
    topPeople: peopleResult.rows.map((r: any) => ({ person: r.person, count: parseInt(r.count) })),
  };
}
